pointsMap_func2 <- function(df,
  ...
  spatial_dataset %>%  full_join(df, join_by(!!spatial_dataset_var == !!var_spatial), keep =
                                   TRUE) -> df_spatial
  ...
  df_spatial %>% rename(
    ...
    filter(!is.na(var)) -> df_spatial
  
 
  min_lon <- min(df_spatial$lon, na.rm = TRUE) - 3
  max_lon <- max(df_spatial$lon, na.rm = TRUE) + 3
  min_lat <- min(df_spatial$lat, na.rm = TRUE) - 3
  max_lat <- max(df_spatial$lat, na.rm = TRUE) + 3
  ...
  plt <- plt + 
    geom_point(data = df_spatial, aes(x = lon, y = lat, size = var, color = var), alpha = 0.8) +
    ...
    viridis::scale_fill_viridis(
      ...
      name = "Landings (1000 t)"
    )
  plt <- plt +  ggrepel::geom_label_repel(
    data = df_spatial,
    aes(lon, lat, label = Harbour),
