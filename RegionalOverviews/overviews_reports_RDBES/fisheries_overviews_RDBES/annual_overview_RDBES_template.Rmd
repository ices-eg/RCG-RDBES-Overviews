...
# species_codes <- read.csv2("../../data/ASFIS_WoRMS.csv")
...
# cl <- cl%>% left_join(species_codes, by = "AphiaID") 
# 
# ######
...
```{r, echo = FALSE, warning = FALSE, message = FALSE, results = "asis", dpi=100, fig.width=6,fig.height=6}
cl_species_c<- cl%>%
  group_by(CLyear, CLspeciesCode, CLlandingCountry) %>%
  summarise(CLscientificWeight_1000ton_total = sum(CLscientificWeight_1000ton))

cl_species_top <- cl_species %>%
  arrange(desc(CLscientificWeight_1000ton)) %>%
  top_n(20)

cl_species_top$Top <- "TOP"
cl_species_c_top <- left_join(cl_species_c,cl_species_top,by="CLspeciesCode")

cl_species_c_top <- cl_species_c_top %>%
  filter(Top=='TOP')

barplot(data = cl_species_c_top,
      x = "CLspeciesCode",
      y = "CLscientificWeight_1000ton",
...
cl_metier_lvl6_top <- cl_metier_lvl6 %>%
  arrange(desc(CLscientificWeight_1000ton)) %>%
  top_n(20)

cl_metier_lvl6_top$Top <- "TOP"
cl_metier_lvl6_c_top <- left_join(cl_metier_lvl6_c,cl_metier_lvl6_top,by="CLmetier6")

cl_metier_lvl6_c_top <- cl_metier_lvl6_c_top %>%
  filter(Top=='TOP')


barplot(data = cl_metier_lvl6_c_top, 
        x = "CLmetier6",
...
cl_harbour_top <- cl_harbour %>%
  arrange(desc(CLscientificWeight_1000ton)) %>%
  top_n(20)

cl_harbour_top$Top <- "TOP"
cl_harbour_fc_top <- left_join(cl_harbour_fc,cl_harbour_top,by="CLlandingLocation")

cl_harbour_fc_top <- cl_harbour_fc_top %>%
  filter(Top=='TOP')


barplot(data = cl_harbour_fc_top, 
        x = "CLlandingLocation",
        y = "CLscientificWeight_1000ton_total",
...
```{r, echo = FALSE, warning = FALSE, message = FALSE, results = "asis", dpi=100, fig.width=6,fig.height=6}
cl_spf<- cl%>%
  filter(CatchGroup=='SPF')%>%
  group_by(CLyear, CLlandingCountry) %>%
  summarise(CLscientificWeight_1000ton = sum(CLscientificWeight_1000ton))
...
#### species
```{r, echo = FALSE, warning = FALSE, message = FALSE, results = "asis", dpi=100, fig.width=6,fig.height=6}
#change for latin name!!!
cl_spf<- cl%>%
  filter(CatchGroup=='SPF')%>%
  group_by(CLyear, CLspeciesFaoCode) %>%
  summarise(CLscientificWeight_1000ton = sum(CLscientificWeight_1000ton))
